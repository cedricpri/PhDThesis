...
In 2016 and 2017 an issue, known as the \textbf{prefiring}, causing highly energetic readout in the \ac{ECAL} endcap to be assigned by the \acf{L1} to the previous bunch crossing was discovered. To make up for this difference, a weight ($1-x$) is usually applied to all \ac{MC} events, where $x$ is the probability of an event to be prefired \cite{Prefire}. The effect this correction has on the data/\ac{MC} agreement in a top control region is shown in Figure~\ref{fig:Prefiring}.
...
Two \acp{WP} are then given directly by the \ac{CMS} EGamma \ac{POG} \cite{ElePOG}, corresponding to an electron selection efficiency of 80 and 90\% respectively. For this analysis, the tight \ac{POG} $mva\_90p\_Iso2016$ \ac{WP} has been chosen for the electron definition, along with additional quality cuts defined in Table~\ref{table:EleWP} (as previously mentionned, these cuts sometines differ quite a bit depending on whether the electron interacts with the \ac{ECAL} barrel ($|\eta| < 1.479$) or one of the endcaps ($|\eta| \geq 1.479$)).
...
\end{tabular}
%}
\caption{Quality cuts applied to define a tight electron in this analysis.}
\label{table:EleWP}
\end{center}
\end{table}	
...
Different control regions have been defined in order for the validity of the simulations performed for the different \ac{SM} processes. 

\subsection{Same sign control region} \label{section:SSCR}

A same sign control region has been defined in order to check the non-prompt background, calculated using a data-driven tight-to-loose method described in Section~\ref{subsection:Fakes}. This \ac{CR} is defined with the following cuts:
...
All the background \ac{MC} samples considered for this analysis are listed in Table~\ref{table:MC}.
...
& DYJetsToLL\_M-50\_TuneCP5\_13TeV-amcatnloFXFX-pythia8 & 6189.38 \\
\hline
\multirow{1}{*}{TTTo2L2Nu} & TTTo2L2Nu\_TuneCP5\_13TeV-powheg-pythia8 & 87.31 \\
\multirow{1}{*}{Single top} & ST\_s-channel\_4f\_leptonDecays\_TuneCP5\_13TeV-madgraph-pythia8 & 3.36 \\
...
\hline
WWTo2L2Nu & WWTo2L2Nu\_NNPDF31\_TuneCP5\_13TeV-powheg-pythia8 & 12.178 \\
\multirow{1}{*}{Zg} & ZGToLLG\_01J\_5f\_TuneCP5\_13TeV-amcatnloFXFX-pythia8 & 131.3 \\ 
\multirow{1}{*}{Wg} & WGToLNuG\_TuneCP5\_13TeV-madgraphMLM-pythia8 & 405.271 \\
\hline
WZTo3LNu & WZTo3LNu\_mllmin01\_NNPDF31\_TuneCP5\_13TeV\_powheg\_pythia8 & 58.59 \\
 \hline
\end{tabular}
}
\caption{Main \ac{MC} simulations for the different background processes considered for this analysis and their respective cross section \color{red}To be updated!\color{black}.}
\label{table:MC}
\end{center}
\end{table}
%\end{landscape}
