...
Different kind of leptons are typically produced by a proton-proton collision. The muons and the electrons can be quite easily identified, mainly because their lifetime and velocity is high enough, meaning that they are not expected to decay inside the \ac{CMS} detector, so they can be directly identified. Taus on the other hand are a bit trickier to deal with because they usually decay inside of the beam pipe itself, resulting in a different reconstruction process. However, since our analysis does not consider taus directly, the details of their reconstruction will not be explained in this section.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=10cm, height=5.5cm]{figs/MuonEff.png}
...
Several algorithms can be used to reconstruct the jets by linking the information coming from the tracker and the calorimeters, but the most used tool in \ac{CMS} is the so-called anti-$k_T$ algorithm, able to cluster all the hadrons produced and merge them into a single jet \cite{JetReco}. This clustering algorithm is able to cluster all the charged and neutral hadrons along with the eventual non-isolated photons or leptons with the objective of computing the energy and direction of the original quark as precisely as possible. 
...
Jets coming from bottom quarks are usually quite interesting to study, especially in this analysis which does rely heavily on the number of b-jets produced to define the control and signal regions, as will be discussed in Chapter~\ref{chapter:Selection}. This specific kind of jets can be distinguished from other jets because of the relatively long lifetime compared to lighter quarks, meaning that b-jets are usually slightly displaced with respect to the \ac{PV} and originating from a secondary vertex, as shown in Figure~\ref{fig:CMSBJets}. This displacement of only a few millimeters of the point of emission of a b-jet is usually enough in order to distinguish this kind of jets with respect to jets coming from lighter jets.
...
Another consequence of the large mass of the bottom quark is that a large number of particles is typically present inside inside this particular kind of jets, and the decay of the bottom quark even leads to the apparition of soft leptons in the decay chain in around 20\% of the cases.

Because of these specific properties of b-jets, algorithm can quite easily distinguish between jets coming from a bottom quark or from lighter quark, and this will be a key point in this analysis. In this case, this discrimination is optimized by using a multivariate technique, able to combine all the discriminating power of the previous typical characteristics of any heavy flavour jet in the best way possible with the objective of being able to identify b-jets as efficiently as possible while reducing the risk of possible misidentification of a jet. 
...
\caption{B-jet identification efficiency and misidentification rate of different b taggers, including the deep CSV b-tag used in this analysis \cite{BTag}.}
\label{fig:CMSBTag}
\end{center}
\end{figure}

Different \acfp{WP} are then made available for all the offline analysis in order to tweak the combo b-jet identification efficiency/misidentification rate. The loose, medium and tight \acp{WP} have been developed in such a way to limit this misidentification rate of a light jet as a b-jet to 10\%, 1\% and 0.1\%, respectively.

\section{\acf{MET}} \label{section:RecoMET}

Since the proton-proton collisions happen mostly head-on, we know that the initial total transverse momentum of the event is exactly equal to 0 before the collision and we expect that it stays 0 afterwards because of the momentum conversation. However, this is not totally true since we are aware of several effects that could induce an imbalance in this transverse momentum:

\begin{itemize}
\item Even though the detector has been carefully designed, some particles could be created outside of the acceptance of the detector and therefore escape the detection (a particle can for example be created with such a boost that it could be emitted back into the beam pipe itself, making it impossible to detect it).
...
The \acf{MET}, a variable defined in Equation~\ref{eq:MET} as the negative sum of the transverse momentum of all the particles $j$ of the event, is able to take into account the eventual imbalance in the transverse momentum due to all the previous effects and is therefore a key variable in most of the analyses searching for new \ac{BSM} physics not expected to interact with the detector.

\begin{equation}
\label{eq:MET}
\overrightarrow{p_T^{\text{miss}}} = - \sum_j \overrightarrow{p_{T, j}}
\end{equation}
