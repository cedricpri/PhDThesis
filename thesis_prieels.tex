...
First of all, I would like to thank the group of high energy physics of the IFCA in general for the opportunity they gave me by offering me such an interesting PhD position within the group. This was a great opportunity for me to start working as a CERN collaborator, which had been a dream of mine for many years. All these years spent at IFCA were for sure extremely interesting, brought me a lot in both the personal and professional points of view, and something I will definitely benefit from in the future.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=5.5cm, height=7cm]{figs/MACHOExclusion.png}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=10cm, height=4.8cm]{figs/STChannels.png}
...
\item Finally, we observed that the variable we called total $E_T$ and corresponding the scalar sum of the total transverse energy of the \ac{MET}, the two leptons and the two b-jets obtained by the top reconstruction process is also able to help with the discrimination process, as shown in Figure~\ref{fig:SRdisc6}.
...
%\item \textbf{Underlying event and parton shower modeling}. The choice of the parton shower generator and \ac{UE} tune is also to have an effect on the final results, however the impact of this systematic is extremely limited in this case because it mostly applies to the WW background, minor in our signal regions.
%\item \textbf{}.
...
\begin{itemize}
\item \textbf{Single top and diboson normalization}. \color{red} Currently not applied? Value? \color{black}
...
The optimization of both the \ac{DNN} and the \ac{BDT} considered i this work was an important part of the analysis, to make sure to use optimized parameters in order to define our network and to get the best discrimination possible between the backgrounds and the two signals of interest, the $t/ \bar t$+DM and the $t \bar t$+DM. In general, such optimization is a complex task to perform because of the high number of hyperparameters that usually influence the quality of the final results. As a starting point, the optimal parameters found by DESY (for the \ac{BDT}) and IFCA (for the \ac{DNN}) during the 2016 analysis were used. 

The global strategy used for the actual optimization process is quite straightforward: even though some of the hyperparameters considered for the optimization are actually related to each other, we decided for the sake of simplicity to treat them as independent and to optimize the network by optimizing each one of these parameters individually. Among all the candidate values for a given parameter, the one leading to the lowest possible validation error while leading to reasonable training times was then simply chosen for the analysis.

\subsubsection{Boosted Decisions Trees}

Starting with the \ac{BDT}, the first obvious parameter among all the parameters typical of a \ac{BDT} and described in Section~\ref{section:BDT} that we decided to optimize was the number of trees, whose impact can be seen in Figure~\ref{fig:optTrees}. In all the plots presented in this section, the signal labeled as "signal0" is the $t \bar t$+DM, while the "signal1" label corresponds to the $t/\bar t$ +DM.
...
\label{fig:optDepth}
\end{figure}
Finally, the last parameter considered during the optimization of the \ac{BDT} $n_{\text{cut}}$ parameter, corresponding to the granularity used when scanning over the variable range to find the optimal splitting criterion, as shown in Figure~\ref{fig:optCuts}.

\begin{figure}[htbp]
\centering
\subfigure[$n_{\text{cut}}$ = 1.000] 
{
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=5.7cm]{figs/placeholder.png}
\end{minipage}\hfill
}
\subfigure[$n_{\text{cut}}$ = 5.000 nodes]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=5.7cm]{figs/placeholder.png}
\end{minipage} \hfill
}
\subfigure[$n_{\text{cut}}$ = 10.000 nodes]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=5.7cm]{figs/placeholder.png}
\end{minipage}\hfill
}
\subfigure[$n_{\text{cut}}$ = 20.000 nodes]
{
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=5.7cm]{figs/placeholder.png}
\end{minipage} \hfill
...
For most of the hyperparameters described so far, we usually expect that increasing (or decreasing) their values will result in better better discrimination results but unfortunately, this usually also increase by a lot the time needed to train the \ac{BDT} so a compromise had to be found for each one of the hyperparameters described here, settling at the end for the parameters described in Table~\ref{table:BDT}.

\subsubsection{Deep Neural Network}

In our particular case, we decided to first of all focus the optimization of the neural network on most of the parameters already described in Section~\ref{section:DNN}, starting by the actual architecture of the network, trying out different number of layers and different number of neurons in each layer, while trying to keep the validation loss obtained to a minimum. The results for the optimization of this first parameter optimization can be found in Figure~\ref{fig:optLayers}. It is also important to note at this point that some of the hyperparameters considered for the optimization are actually related to each other but for the sake of simplicity, we decided to treat them as independent and to optimize the network by optimizing each one of these parameters individually.
...
\subfigure[Hidden layers 20/15/5]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/placeholder.png}
\end{minipage} \hfill
...
\caption{Optimization of the \ac{DNN} used in the analysis, trying out different network  architectures, from the simplest (on the top left) to the most complex (on the bottom right).}
\label{fig:optLayers}
\end{figure}

ADD CONCLUSION-> decided to keep...

After this first step of optimization, we kept the architecture which seemed to give the best results and started a subsequent optimization process, evaluating this time: i) the optimizer with different learning rates (Figure~\ref{fig:optOptimizer} and ii) the batch size parameter (Figure~\ref{fig:batch}).


the number of epochs (by a simple trial and error simply by making sure that the results did not improve for a long time 

number of epochs, batch size

Similar for the BDT.

Input variables. All normalized.

\color{red} FIXME: did we end up using the DNN or the BDT? \color{black}
