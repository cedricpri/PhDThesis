...
Several different \ac{SM} background processes have been considered for this analysis, all listed in Section~\ref{appendix:BkgSamples} and mostly estimated directly from \ac{MC}. In this section, the main backgrounds to consider for this particular analysis will be quoted, such as:

\begin{itemize}
\item The major background for the $t \bar t$+DM analysis is the \ac{SM} $t \bar t$, kinematically close to the signal searched for (Section~\ref{subsection:ttbar}). 
\item On the other hand, the major background for the $t/\bar t$ analysis is the single top production, which has an even higher cross section than the $t \bar t$ (Section~\ref{subsection:singleTop}).
\item Then, mainly because of its huge cross section shown in Figure~\ref{fig:ProcessesXS}, the \ac{DY} process is usually quite important, even in the signal regions (Section~\ref{subsection:DY}). 
\item The non-prompt background, or fakes, is another important piece of this analysis mainly because of the particular data-driven method used to compute them (Section~\ref{subsection:Fakes}).
\item Fianlly, the $t \bar t + V$ ($t \bar t + Z$ and $t \bar t + W$) may have a kinematics even closer to our signal than the $t \bar t$ process and is therefore extremely important in our signal regions, even though its low cross section does limit its impact (Section~\ref{subsection:ttV}).
\end{itemize}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=7cm, height=8cm]{figs/ProcessesXS.png}
...
Finally, some smaller backgrounds will be introduced in Section~\ref{subsection:SmallerBkg} and the weights and corrections aplied to all these \ac{MC} samples will be detailed in Section~\ref{subsection:Weights}.

\color{red} TALK ABOUT SINGLE TOP? \color{black}

\subsection{Top production}

Dominant in both searches
...
Different Feynman diagrams contribute to this process at \ac{LO}, as shown in Figure~\ref{fig:ttbarProd}.
...
This background is so similar to the signal searched for that some \ac{ML} techniques had to be developed in order to find some discrimination between the two processes.

\subsubsection{Single top} \label{subsection:singleTop}

s-channel in Figure~\ref{fig:singleTopSChann}
other in Figure~\ref{fig:singleTopOtherChann}
...
qcd?
w+jets and z+jets in Figure~\ref{fig:FakesW}
...
\end{minipage} 
\caption{Possible Feynman diagrams for the production of a W/Z boson in association with a jet.}
\label{fig:FakesW}
\end{figure}
...
Diboson contamination Figure~\ref{fig:DiBosons}.
...
\subsection{Muon \ac{WP}}
\subsection{Jet \ac{WP}}
