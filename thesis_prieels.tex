...
\begin{titlepage}
	...
	\vspace{2.6cm}
	{\Large Santander, September 2021}
	\vfill
...
\begin{titlepage}
	...
	\textbf{Pablo Mart\'{i}nez Ru\'{i}z del \'{A}rbol}\\}
	\vspace{2.6cm}
	{\Large Santander, Septiembre 2021}
	\vfill
...
Of course, I would especially like to thank my parents Philippe and Chantal, and my brothers Antoine and Lucas for giving me the opportunity to come and live in Spain for so long. I never could have reached this far without your presence and support that I felt daily, even more than a thousand kilometers away. Thank you to In\'{e}s as well, as I would never have started this thesis if I hadn't met you. Finally, I would like to especially thank Aline for her incredible support over the past few months, which helped me a lot and for which I am extremely grateful. Even though one chapter now ends, I cannot wait to get started on the next one, and I sure hope that this is only the beginning with you.
...
\begin{minipage}[b]{.49\textwidth}
\begin{center}
\includegraphics[width=7.5cm, height=5.6cm]{figs/Rinout2016.png}
\end{center}
\end{minipage} \hfill
\begin{minipage}[b]{.49\textwidth}
\begin{center}
\includegraphics[width=7.5cm, height=5.6cm]{figs/Rinout2017.png}
\end{center}
\end{minipage} \hfill
\begin{minipage}[b]{.99\textwidth}
\begin{center}
\includegraphics[width=7.5cm, height=5.6cm]{figs/Rinout2018.png}
\end{center}
\end{minipage} \hfill
\caption{$R_{out/in,\text{ }} MC$ transfer factor obtained with the Rin-out data-driven method in bins of \ac{MET} in 2016 (on the top left), 2017 (on the top right) and 2018 (on the bottom).}
...
Even though we initially planed on applying this correction factor, the most recent recommendation is not to apply this factor for our kind of \ac{BSM} searches. In this case, the effect of the top $p_T$ mismodeling can be considered covered by the existing uncertainties and no additional correction or uncertainty is needed \cite{topPtReweighting}.
...
\caption{Data/\ac{MC} agreement without (on the left) and with (on the right) prefiring corrections in a general 2017 top control region.}
\label{fig:Prefiring}
\end{center}
\end{figure}

In 2018, another issue affecting this time two endcaps of the \ac{HCAL} and known as the HEM15/16 issue was reported \cite{HEM}, resulting in a loss of around 2\% of \ac{HCAL} coverage. This issue has a small but measurable effect on the \ac{MET} spectrum and is therefore taken into account and corrected by introducing another weight to the simulation.
...
We already described what to expect from a typical $t/\bar t$ or $t \bar t$+DM signal: the typical signature of such signals is made out of a certain number of b tagged jets along with two leptons (electrons and/or muons) and some \ac{MET} coming from the two \ac{DM} particles created along the way. It is extremely important to describe the \ac{WP} chosen and the selection applied in order to select the objects of the analysis, such as the leptons and the jets used, chosen in such a way to optimize the lepton reconstruction efficiency while reducing as much as possible the possible misidentification rates of the different objects.
...
The triggers, described in Section~\ref{subsection:Trigger}, and particularly the trigger paths chosen are an important part of each analysis since they describe the kind of data that can be collected and therefore analyzed. The triggers used in this analysis for the datasets available for the years 2016, 2017 and 2018 can be found in Tables~\ref{table:Trigg2016},~\ref{table:Trigg2017} and~\ref{table:Trigg2018} respectively.
...
In particular, the trigger efficiency, defined for each trigger as the ratio between the number of events passing our object selection and the trigger itself in the numerator and the number of events passing our selection in the denominator has been computed using orthogonal \ac{MET} datasets to avoid any bias. Studying this efficiency is important is the sense that we want to make sure that the triggers used are efficient enough in the $p_T$ region of the leptons of the analysis to avoid any undesired effect due to the turn-on of any trigger, and because we actually use the computed efficiency to reweight the simulated samples. The efficiencies have in this sense been calculated using orthogonal \ac{MET} triggers for the different data taking periods and for each channel individually, as shown in Figure~\ref{fig:TriggEff}, following the recommendations of \cite{triggerEff}. At the end of the day though, 2D efficiencies (the x-axis being the leading lepton $p_T$ ($ee$ and $\mu \mu$ channels) or the electron $p_T$ ($e \mu$ channel), while the trailing lepton $p_T$ ($ee$ and $\mu \mu$ channels) or the muon $p_T$ ($e \mu$ channel) was used to define the y-axis) were actually estimated in both data and \ac{MC}. A scale factor was then computed and used in the analysis to compute the weight of each event that needs to be applied to account for this effect.
...
Finally, a slightly different selection (tight \ac{PU} jet ID) is applied to jets having a $p_T < 50$ GeV in order to reject jets coming from \ac{PU} interactions. Following the officical \ac{POG} recommendations, jet energy correction and resolution are also taken into account \cite{JEC2, JER}.
...
First of all, a global pre-selection was agreed on between the groups involved, applied to all the signal regions, reducing by a large factor the contamination due to several backgrounds, such as the \ac{SM} $t \bar t$ and the \ac{DY} processes:
...
\item Then, events with a third lepton having a $p_T > 10$ GeV are rejected;
\item Low mass resonances are removed by asking for $m_{ll} > 20$ GeV;
\item The \ac{DY} is then strongly reduced by asking for a 15 GeV Z-veto in the $ee$ and $\mu \mu$ channels, and by asking for at least 1 jet with a $p_T > 30$ GeV and $|\eta| < 2.4$ in the event;
\item 1 medium deep CSV b-jet is also required;
\item And, finally, a cut using the stranverse mass (defined in Section~\ref{section:Variables}) $M_{T2}(ll) > 80$ GeV has been applied, mainly to keep our signal regions orthogonal to the $t \bar t$ control regions used by the semi-leptonic channel, making it easier to combine our results while removing more background than signal anyway.
\end{itemize}
...
\subfigure[2016 pre-selection region]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/log_cratio_topCR_ll_METcorrected_pt.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/log_cratio_topCR_ll_mt2ll.png}
\end{minipage} \hfill
}
\subfigure[2017 pre-selection region]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/log_cratio_topCR_ll_METcorrected_pt.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
...
Several different signal regions have then actually been defined, targeting each of the two signals of interest, depending on the number of jets and b-jets observed. The signal region targeting the $t/\bar t$+DM process is therefore made out of events having either exactly 1 jet, or 2 jets and exactly 1 b-jet. On the other hand, all the events passing the pre-selection but having either 2 jets and more than 1 b-jet or more than 2 jets make up the signal region targeting the $t \bar t$+DM process. Different \ac{MVA} trainings were done in these two regions, as explained in Section~\ref{section:Discrimination}. The signal regions can be even more divided, depending on the mass point that was used for the training, since the mediators with different masses typically feature different kinematic distributions.

Some distributions obtained in these signal regions are shown in Figures~\ref{fig:SR1} to~\ref{fig:SR6}.

\color{red}FIXME: Add final plots everywhere \color{black}

\begin{figure}[htbp]
\centering
\subfigure[2016 scalar 100 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/placeholder.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/placeholder.png}
\end{minipage} \hfill
}
\subfigure[2017 scalar 100 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/placeholder.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/placeholder.png}
\end{minipage} \hfill
}
\subfigure[2018 scalar 100 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/placeholder.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/placeholder.png}
\end{minipage} \hfill
}
\caption{Two different variables (pf\ac{MET}, on the left, and the stransverse mass $M_{T2}^{ll}$, on the right) represented in the $t/\bar t$+DM signal region defined from the 100 GeV scalar training.}
\label{fig:SR1}
\end{figure}

\begin{figure}[htbp]
\centering
\subfigure[2016 scalar 500 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/placeholder.png}
\end{minipage}\hfill
...
\subfigure[2017 scalar 500 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/placeholder.png}
\end{minipage}\hfill
...
\subfigure[2018 scalar 500 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/placeholder.png}
\end{minipage}\hfill
...
\caption{Two different variables (pf\ac{MET}, on the left, and the stransverse mass $M_{T2}^{ll}$, on the right) represented in the $t/\bar t$+DM signal region defined from the 500 GeV scalar training.}
\label{fig:SR2}
\end{figure}
...
\caption{Two different variables (pf\ac{MET}, on the left, and the stransverse mass $M_{T2}^{ll}$, on the right) represented in the $t/\bar t$+DM signal region defined from the 100 GeV pseudoscalar training.}
\label{fig:SR3}
\end{figure}
...
\section{Background estimation and control regions} \label{section:CR}
...
\begin{figure}[htbp]
\centering
\subfigure[2016 $t \bar t$+V \ac{CR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/log_cratio_ttVCR_ll_mll.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/log_cratio_ttVCR_ll_METcorrected_pt.png}
\end{minipage} \hfill
}
\subfigure[2017 $t \bar t$+V \ac{CR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/log_cratio_ttVCR_ll_mll.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/log_cratio_ttVCR_ll_METcorrected_pt.png}
\end{minipage} \hfill
}
\subfigure[2018 $t \bar t$+V \ac{CR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_ttVCR_ll_mll.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
...
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_dark_pt.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_DNN_signal0_scalar100_dark_pt.png}
\end{minipage} \hfill
}
...
\centering
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_overlapping_factor.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_DNN_signal0_scalar100_overlapping_factor.png}
\end{minipage} \hfill
}
\caption{Dark $p_T$ and overlapping factor $R$ distribution in the 2018 pre-selection (on the left) and scalar 100 GeV signal (on the right) regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdisc1}
\end{figure}
...
\centering
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_nbjet.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_DNN_signal0_scalar100_nbjet.png}
\end{minipage} \hfill
...
\centering
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_dphillmet.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_DNN_signal0_scalar100_dphillmet.png}
\end{minipage} \hfill
...
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_DNN_signal0_scalar100_MET_significance.png}
\end{minipage} \hfill
...
\centering
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_reco_weight.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_DNN_signal0_scalar100_reco_weight.png}
\end{minipage} \hfill
...
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_r2l.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_DNN_signal0_scalar100_r2l.png}
\end{minipage} \hfill
}

\subfigure[$r2l4j$ variable] {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_r2l4j.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_DNN_signal0_scalar100_r2l4j.png}
\end{minipage} \hfill
}
...
\item We also observed that the variable \textit{massT} which corresponds to the scalar sum of the transverse component of the pf\ac{MET}, the two leptons and the two b-jets obtained by the top reconstruction process helps with the discrimination process, as shown in Figure~\ref{fig:SRdisc6}.
...
\subfigure[Scalar mediators] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=4.5cm]{figs/ROC_scalar100.png}
\end{minipage} \hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=4.5cm]{figs/ROC_scalar500.png}
\end{minipage} \hfill
}

\subfigure[Pseudoscalar mediators] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=4.5cm]{figs/ROC_pseudo100.png}
\end{minipage} \hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=4.5cm]{figs/ROC_pseudo500.png}
\end{minipage} \hfill
}
\caption{Signal versus background \ac{ROC} curves obtained after the training performed, considering 100 (on the left) and 500 GeV (on the right) mediators.}
\label{fig:ROC}
\end{figure}
...
\centering
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7.8cm, height=5.8cm]{figs/groupedROC_scalar.png}
\end{minipage} \hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7.8cm, height=5.8cm]{figs/groupedROC_pseudo.png}
\end{minipage} \hfill
\caption{Signal versus background \ac{ROC} curves obtained after the training performed with the \ac{ANN}, considering all the scalar (on the left) and pseudoscalar (on the right) mediators available.}
\label{fig:ROC2}
\end{figure}

\begin{figure}[htbp]
\centering
\subfigure[Scalar overtraining plot] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=5cm]{figs/overtraining_scalar100.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=5cm]{figs/overtraining_scalar500.png}
\end{minipage}\hfill
}

\subfigure[Pseudoscalar overtraining plot] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=5cm]{figs/overtraining_pseudo100.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=6.5cm, height=5cm]{figs/overtraining_pseudo500.png}
\end{minipage}\hfill
}
\caption{Overtraining distributions obtained for the 100 (on the left) and 500 GeV (on the right) mediators.}
...
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/log_cratio_topCR_ll_var_DNN_signal0_scalar100.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/log_cratio_topCR_ll_var_DNN_signal0_scalar500.png}
\end{minipage} \hfill
}

\subfigure[2017 distributions] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/log_cratio_topCR_ll_var_DNN_signal0_scalar100.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/log_cratio_topCR_ll_var_DNN_signal0_scalar500.png}
\end{minipage} \hfill
}

\subfigure[2018 distributions] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_var_DNN_signal0_scalar100.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_var_DNN_signal0_scalar500.png}
\end{minipage} \hfill
}
\caption{\ac{ANN} output distribution used in the shape analysis performed later on, for the scalar 100 GeV (on the left) and 500 GeV (on the right) training.}
\label{fig:ANNShapesScalar}
\end{figure}
...
\subfigure[2016 distributions] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/log_cratio_topCR_ll_var_DNN_signal0_pseudo100.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/log_cratio_topCR_ll_var_DNN_signal0_pseudo500.png}
\end{minipage} \hfill
}

\subfigure[2017 distributions] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/log_cratio_topCR_ll_var_DNN_signal0_pseudo100.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/log_cratio_topCR_ll_var_DNN_signal0_pseudo500.png}
\end{minipage} \hfill
}

\subfigure[2018 distributions] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_var_DNN_signal0_pseudo500.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/log_cratio_topCR_ll_var_DNN_signal0_pseudo500.png}
\end{minipage} \hfill
...
\item \textbf{Non-prompt background}. A normalization and shape uncertainty is associated to the estimation of this background in a specific control region according to the tight-to-loose method previously described. The combined uncertainty due to statistics and the impact of a change in the input jet $E_T$ threshold chosen has been estimated to be less than $\sim15$\%, while an additional flat 30\% systematic is considered for this background to cover for eventual discrepancies between data and \ac{MC} in the same-sign control region defined in Section~\ref{section:SSCR}.
\end{itemize}
...
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/limit_scalar2016.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/limit_pseudo2016.png}
\end{minipage} \hfill
}

\subfigure[2017 exclusion limits] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/limit_scalar2017.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/limit_pseudo2017.png}
\end{minipage} \hfill
}

\subfigure[2018 exclusion limits] {
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/limit_scalar2018.png}
\end{minipage}\hfill
\begin{minipage}[b]{.49\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/limit_pseudo2018.png}
\end{minipage} \hfill
}
...
\begin{itemize}
\item Las \textbf{b\'{u}squedas directas}, buscando posibles interacciones entre materia oscura y part\'{i}culas del Modelo Est\'{a}ndar, estudiando por ejemplo choques y por lo tanto cambio de energ\'{i}a entre estos dos sectores.
...
La mayor\'{i}a de los fondos se estiman directamente a partir de simulaciones mat\'{e}maticas de tipo Monte-Carlo, y se pueden controlar en zonas de control enriquidas en fondos en particular. A\'{u}n as\'{i}, dos fondos tienen un tratamiento un pocp peculiar:

\begin{itemize}
\item En el caso del Drell-Yan, se usan los datos mismos para calcular un factor de correci\'{o}n que permite estimar el n\'{u}mero de eventos de este proceso fuera del pico del Z a partir de los eventos observados dentro de este pico.
\item Por otro lado, algunos procesos como el W+jets que solo tiene un lept\'{o}n en el estado final tambi\'{e}n puede llevar a contaminar la zona de se\~{n}al por culpa de los \textit{fakes}, que suelen ser objetos como jets misidentificados por el detector como leptones, o leptones que no provienen del Primary Vertex de la colisi\'{o}n prot\'{o}n-prot\'{o}n. La estimaci\'{o}n de este fondo se hace mediante un m\'{e}todo data-driven ya que se no se espera poder simular este tipo de fonde de manera optimal usando simulaciones de Monte-Carlo, y se controla este fondo peculiar en una zona enrichida en fakes, definida con dos leptones que tengan el mismo signo de carga.
\end{itemize}
...
El resultado m\'{a}s importante de este trabajo consiste en pintar los limites de exclusi\'{o}n de la secci\'{o}n eficaz de producci\'{o}n de las se\~{n}ales de int\'{e}res, para las dos se\~{n}ales y para los 3 a\~{n}os de toma de datos por separado, y luego combinando todos estos par\'{a}metros. Esto se hace considerando diferentes fuentes de errores estad\'{i}sticos y sistem\'{a}ticos, ambos te\'{o}ricos (tal y como el sistem\'{a}tico relacionado con la estimaci\'{o}n de las \ac{PDF} de los diferentes procesos) y experimentales (tal y como el error asociado a la medida de la luminosidad colectada por el detector \ac{CMS}).
...
\hline
\multirow{1}{*}{Drell-Yan} & DYJetsToLL\_M-10to50\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 ($H_T < 70$ GeV) & 15810.0 \\
...
& DYJetsToLL\_M-50\_HT-70to100\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 & 146.5 \\
& DYJetsToLL\_M-50\_HT-100to200\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 & 160.7 \\
& DYJetsToLL\_M-50\_HT-200to400\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 & 48.58 \\
& DYJetsToLL\_M-50\_HT-400to600\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 & 6.923 \\
& DYJetsToLL\_M-50\_HT-600to800\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 & 1.717 \\
& DYJetsToLL\_M-50\_HT-800to1200\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 & 0.7275 \\
& DYJetsToLL\_M-50\_HT-1200to2500\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 & 0.1614 \\
& DYJetsToLL\_M-50\_HT-2500toInf\_TuneCUETP8M1\_13TeV-madgraphMLM-pythia8 & 0.003566 \\
\hline
...
\begin{tabular}{ c|c|c } 
 ...
 Process & Sample & Cross section [pb] \\
\hline
\multirow{1}{*}{Drell-Yan} & DYJetsToLL\_M-10to50\_TuneCP5\_13TeV-madgraphMLM-pythia8 ($H_T < 100$ GeV) & 15810 \\
...
& DYJetsToLL\_M-50\_HT-70to100\_TuneCP5\_PSweights\_13TeV-madgraphMLM-pythia8 & 146.5 \\
& DYJetsToLL\_M-50\_HT-100to200\_TuneCP5\_13TeV-madgraphMLM-pythia8 & 160.7 \\
& DYJetsToLL\_M-50\_HT-200to400\_TuneCP5\_13TeV-madgraphMLM-pythia8 & 48.58 \\
& DYJetsToLL\_M-50\_HT-400to600\_TuneCP5\_13TeV-madgraphMLM-pythia8 & 6.923 \\
& DYJetsToLL\_M-50\_HT-600to800\_TuneCP5\_13TeV-madgraphMLM-pythia8 & 1.717 \\
& DYJetsToLL\_M-50\_HT-800to1200\_TuneCP5\_13TeV-madgraphMLM-pythia8 & 0.7275 \\
& DYJetsToLL\_M-50\_HT-1200to2500\_TuneCP5\_13TeV-madgraphMLM-pythia8 & 0.1614 \\
& DYJetsToLL\_M-50\_HT-2500toInf\_TuneCP5\_13TeV-madgraphMLM-pythia8 & 0.003566 \\
\hline
...
The global strategy used for the actual optimization process is quite straightforward: even though some of the hyper-parameters considered are actually related to each other, we decided for the sake of simplicity to treat them as independent and to optimize the network by optimizing each one of these parameters individually. Among all the candidate values for a given parameter, the one leading to the best possible ROC curve while keeping the time needed to train the network reasonably low was then simply chosen for the analysis. The training optimization has been performed for the 100 GeV scalar mediators for both signal processes, an intermediate mass point that should feature some of the lowest discrimination between the signals and the backgrounds, given their kinematics similarities and a similar number of background and signal events was used to train the different methods. %In all the plots presented in this section, corresponding to the most significant hyperparameters which were found to have a significant impact, the signal labeled as \textit{signal0} is the $t \bar t$+DM, while the \textit{signal1} label corresponds to the $t/\bar t$ +DM.

\subsubsection{Boosted Decisions Trees}

Starting with the \ac{BDT}, the first obvious hyper-parameter among all the parameters typical of a \ac{BDT} and described in Section~\ref{section:BDT} that we decided to optimize was the number of trees, whose impact on the final discrimination obtained between the processes can be seen in the \ac{ROC} curves shown in Figure~\ref{fig:optTrees}.
...
% dark pt/overlapping factor, spin correlated, atlas
%
%\begin{itemize}
%\item The \textbf{set 1} is the simple set of variables possible, made out of 5 variables: both the number of b-jets and $m_{bl}^t$, to discriminate the two signals with respect to each other, and the \ac{PUPPI} \ac{MET}, $\Delta \Phi(E_{T}^{\text{miss}}, ll)$ and $M_{T2}^{ll}$ to discriminate the signals with respect to the backgrounds considered, the \ac{SM} single top and $t \bar t$ processes.
%\item The \textbf{set 2} is obtained by adding both the \ac{MET} significance and the $m_{T2}(bl, bl)$ variables to the set 1.
%\item Then, the \textbf{set 3} is defined as the set 2, to which the massT and reo weights variables have been added.
%\item The \textbf{set 4}
%\end{itemize}

%\begin{figure}[htbp]
%\centering
%\begin{minipage}[b]{.48\textwidth}
%\includegraphics[width=7cm, height=5.7cm]{figs/correlations_background.png}
%\end{minipage}
%\begin{minipage}[b]{.48\textwidth}
%\includegraphics[width=7cm, height=5.7cm]{figs/correlations_signal.png}
%\end{minipage}
%\caption{Correlation observed in the background (on the left) and signal (on the right) processes between all the possible input variables considered.}
%\label{fig:allVarCorr_bkg}
%\end{figure}

%\begin{figure}[htbp]
%\centering
%\includegraphics[width=7cm, height=5.7cm]{figs/corr_allVariables_signal1.png}
%\caption{Correlation observed in the $t/\bar t$+DM process between all the possible input variables considered.}
%\label{fig:allVarCorr_signal1}
%\end{figure}
%
%\begin{figure}[htbp]
%\centering
%\includegraphics[width=7cm, height=5.7cm]{figs/corr_allVariables_signal0.png}
%\caption{Correlation observed in the $t \bar t$+DM process between all the possible input variables considered.}
%\label{fig:allVarCorr_signal0}
%\end{figure}
...
\begin{center}
\includegraphics[width=12cm, height=10cm]{figs/impactsScalar100.png}
\end{center}
\end{minipage}\hfill
}
\subfigure[500 GeV] 
...
\begin{minipage}[b]{1.0\textwidth}
\begin{center}
\includegraphics[width=12cm, height=10cm]{figs/impactsScalar500.png}
\end{center}
\end{minipage} \hfill
}
...
\begin{minipage}[b]{1.0\textwidth}
\begin{center}
\includegraphics[width=12cm, height=10cm]{figs/impactsPseudo100.png}
\end{center}
\end{minipage} \hfill
}
\subfigure[500 GeV] 
...
\begin{minipage}[b]{1.0\textwidth}
\begin{center}
\includegraphics[width=12cm, height=10cm]{figs/impactsPseudo500.png}
\end{center}
\end{minipage} \hfill
}
