...
\color{red} FIXME: Add correct plots for top pt reweighting when available \color{black}
...
Let's start with the two variables we found to be the most useful in order to separate the $t$+DM and $t \bar t$+DM signals. Since both signatures have a different number of top quarks in their final state, they are also expected to lead to a different number of observed b-jets, so this quantity is an obvious choice for a good discriminating variable. However, simply events featuring two b-jets or more to select a region enriched in $t$+DM signal turns out in practice to be an ineffective strategy, mainly since the b-tagger efficiency for our chosen working point is of the order of 85\% only, as discussed in Section~\ref{section:BTag}, which can result in a large surviving $t \bar t$+DM background, especially given the difference in cross-section between the two processes.
...
\end{minipage} \hfill
\caption{Number of b-jets (on the left) and $m_{bl}^t$ variable (on the right), mostly used to separate our two signals in this analysis, in a control region close to the actual signal region, for different signal samples of the analysis.}
\label{fig:signalsDiscrimination}
\end{figure}

As we can see in these plots, these variables do offer us some discriminations between the different signals considered but even by using them, we cannot define a signal region enriched in $t$+DM because of the low cross-section of this process.
...
\end{minipage}\hfill
\caption{\ac{PUPPI} \ac{MET} distribution in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdiscMET}
\end{figure}
...
\end{minipage} \hfill
\caption{$M_{T2}(ll)$ distribution in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdiscMT2}
\end{figure}
...
\end{minipage} \hfill
\caption{$M_{T2}(bl, bl)$ distribution in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
...
\end{minipage} \hfill
\caption{Dark $p_T$ distribution in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdisc1}
\end{figure}
...
\end{minipage} \hfill
\caption{Overlapping factor $R$ distribution in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdisc2}
\end{figure}
...
\end{minipage} \hfill
\caption{Cosine of the full opening angle of the top decay products in their respective parent rest frames in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdisc3}
\end{figure}
...
\begin{itemize}
\item $\Delta \Phi(\cancel{E}_{T}^{\text{miss}}, ll)$: the distribution in $\Phi$ of the two leptons is expected to change depending on the eventual production of \ac{DM}, as shown in Figures~\ref{fig:scheme_deltaphi} and~\ref{fig:SRdisc4}. 
...
\end{minipage} \hfill
\caption{$\Delta \Phi(\cancel{E}_{T}^{\text{miss}}, ll)$ distribution in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdisc4}
\end{figure}
...
\end{minipage} \hfill
\caption{Kinematic reconstruction weight $W$ distribution in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdisc5}
\end{figure}

\item Finally, we observed that the variable we called \textit{totalET} and corresponding the scalar sum of the total transverse energy of the \ac{MET}, the two leptons and the two b-jets obtained by the top reconstruction process is also able to help with the discrimination process, as shown in Figure~\ref{fig:SRdisc6}.
...
\end{minipage} \hfill
\caption{Scalar sum of the transverse energy of the particles produced in the events distribution in the $t$+DM (on the left) and $t \bar t$+DM 2018 signal regions (on the right) for the backgrounds and several signal samples.}
\label{fig:SRdisc5}
\end{figure}
\end{itemize}
