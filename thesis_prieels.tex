...
When taking into account all these possible combinations, a reconstruction efficiency of $\sim 96$\% has been achieved when considering standard $t \bar t$ \ac{MC} samples.

This efficiency can be slightly increased even more by performing a \textbf{smearing} method, taking into account imperfectly measured kinematic variables by repeating the reconstruction 100 times for each combination previously defined, by updating in each iteration several parameters:
\begin{itemize}
\item First of all, the jets and leptons energies are updated within their respective uncertainties, and the \ac{MET} is recomputed each time using these newly calculated parameters.
\item Then, an angular smearing is also performed, using the angles $\alpha$ and $\omega$ shown in Figure~\ref{fig:angsmearing}. 
...
The $\omega$ angle is first of all taken from a simple uniform distribution, while $\alpha$ is determined from simulation of the \ac{SM} $t \bar t$ process, for b-jets and leptons separetely, as shown in the distributions in Figure~\ref{fig:angdistributions}.
...
\centering
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=6.3cm, height=4.7cm]{figs/placeholder.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=6.3cm, height=4.7cm]{figs/placeholder.png}
\end{minipage} \hfill
\caption{Simulated distributions of the $\alpha$ angle between the particle and detector level direction for jets (on the left) and letpons (on the right).}
\label{fig:angdistributions}
\end{figure}

During each iteration of the smearing process, an individual weight $w$ is computed from the true invariant mass $m_{lb}$ distribution previously obtained using generation. Both the W mass distribution obtained and this distribution are represented in Figure~\ref{fig:truemlb}.

\begin{figure}[htbp]
\centering
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=6.3cm, height=4.7cm]{figs/bw.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=6.3cm, height=4.7cm]{figs/mlb2.png}
\end{minipage} \hfill
...
Already introduced with the top reconstruction method in Section~\ref{section:ttrecoDM}, these two variables are by constructions expected to give some discrimination between the signal and the different background processes, mainly the $t \bar t$. Indeed, the dark $p_T$ is expected to take slightly higher values for the signal, where we expect the ellipses to be further apart from each other due to the production of \ac{DM}.

The overlapping factor $R$ defined in Equation~\ref{eq:overlapping}, on the other hand, is expected to peak around 1 for the standard $t \bar t$ process (since in this case, we expect to have $d = l_1 + l_2$ for most of the events, as seen in Figure~\ref{fig:ellipsesDM}), and to take slightly lower values for the different signal mass points considered.
