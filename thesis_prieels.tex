...
One training has been done for each mediator mass, considering separately the scalar and pseudoscalar mediators in order to enhance the sensitivity of the analysis. Each \ac{ANN} has been trained in order to reduce as much as possible the dependence on the sample itself, by weighting the samples according to the specific cross-section of each mass point. 

For the training process itself, all the signals for a given mass point have been mixed together and trained against all the possible backgrounds, in the pre-selection region defined in Section~\ref{section:SR}. Two different trainings were actually performed for each mass point:
\begin{itemize}
\item One targeting specifically the $t/\bar t$+DM signal, by considering only the events having 1 or 2 jets and exactly 1 b-jet;
...
Once the weights applied, we have been able to obtain a distribution of the most probably category for all the processes considered in this analysis, as shown in Figure~\ref{fig:weightsApplied}, and the actual \ac{ANN} output distributions for each mediator mass point, used in a general shape analysis later on, as shown in Figures~\ref{fig:ANNShapesScalar} and \ref{fig:ANNShapesPseudo}.
...
The \ac{ROC} curves for each set of input variables have been obtained and plotted on top of each other in order to simplify their comparison, as shown in Figure~\ref{fig:InputVarROC}. At the end of the day, the set of input variables number XX was chosen over the others as it seems to be giving the best possible signal over background discrimination.
...
{
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=5.7cm]{figs/BDT_input_ST.png}
\end{minipage}\hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=5.7cm]{figs/BDT_input_TTbar.png}
\end{minipage}\hfill
}
\subfigure[\ac{ANN} ROC curves] 
{
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=5.7cm]{figs/ANN_input_ST.png}
\end{minipage} \hfill
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=5.7cm]{figs/ANN_input_TTbar.png}
\end{minipage} \hfill
}
...
Finally, we compared the \ac{ROC} curves obtained with the best \ac{BDT} and the best \ac{ANN} together, in order to choose a singla \ac{MVA} method to perform this analysis. The result of this particular comparison is shown in Figure~\ref{fig:MVAcomparison}.
...
\end{minipage}\hfill
\caption{Background rejection achieved for the \ac{BDT} and the \ac{ANN}, for the training done in the $t/ \bar t$+DM (on the left) and $t \bar t$+DM (on the right) regions.}
\label{fig:MVAcomparison}
\end{figure}

According to all these results, we actually decided to use the \ac{BDT} for the analysis instead of the \ac{ANN}, given they it achieves a much higher background rejection when rejecting the same level of signal. The method was trained with the most complete set of input variables, since this seems to be the set of variables able to give us the best possible discrimination between the signals and the backgrounds around 100 GeV, the mass point chosen for the optimization process.
