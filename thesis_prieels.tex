...
\begin{itemize}
\item The major background of this analysis is the \ac{SM} $t \bar t$, kinematically really close to the $t \bar t$+DM signal searched for (Section~\ref{subsection:ttbar}). The single top production is also extremly important to consider, being kinematically close to our $t/\bar t$+DM signal, having an even higher cross section than the $t \bar t$ (Section~\ref{subsection:singleTop}), mainly because of the variety of channels resulting in such a final state. Asking for two jets does allow us to reduce this background impact, while unfortunately leaving some signal behind in the process.
...
En el anillo principal de la cadena de acceleradores formando el \ac{LHC}, 4 detectores han sido construidos alrededor del \textit{beam pipe} para estudiar las part\'{i}culas producidas despu\'{e}s de las colisiones entre los protones. Estos detectores, \ac{CMS}, \ac{ATLAS}, \ac{ALICE} y LHCb, han sido desarollados usando equipos y m\'{e}todos distintos, para lograr varios objetivos. 
