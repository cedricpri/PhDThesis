...
Several different signal regions have then actually been defined, one for each training performed (more details will be given regarding this process in Section~\ref{section:Discrimination}):
\begin{itemize}
...
\begin{figure}[htbp]
\centering
\subfigure[2016 pseudoscalar 100 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/placeholder.png}
\end{minipage}\hfill
...
\subfigure[2017 pseudoscalar 100 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/placeholder.png}
\end{minipage}\hfill
...
\subfigure[2018 pseudoscalar 100 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/placeholder.png}
\end{minipage}\hfill
...
\begin{figure}[htbp]
\centering
\subfigure[2016 pseudoscalar 500 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2016/placeholder.png}
\end{minipage}\hfill
...
\subfigure[2017 pseudoscalar 500 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2017/placeholder.png}
\end{minipage}\hfill
...
\subfigure[2018 pseudoscalar 500 GeV \ac{SR}]
 {
\begin{minipage}[b]{.48\textwidth}
\includegraphics[width=7cm, height=7cm]{figs/2018/placeholder.png}
\end{minipage}\hfill
...
Different control regions where the data/\ac{MC} agreement can be checked have been defined in order to check the validity of the \ac{MC} simulations performed for the \ac{SM} processes corresponding to the main backgrounds of this analysis. In this context, an inclusive control region is defined in Section~\ref{section:inclusiveCR} to spot initial issues and problems. A \ac{DY} enriched specific control region is also defined in Section~\ref{section:DYCR} while a data validation region to ensure the quality of the \ac{MC} prediction of both the $t \bar t$ and single top processes in Section~\ref{section:TopCR} is defined.% Finally, the $ttV$ and non-prompt contamination processes will be checked in Sections~\ref{section:ttVCR} and~\ref{section:SSCR} respectively.
...
It is also important to note that some of these variables rely on the information obtained from the top reconstruction performed, which might fail in some cases. In this case, a default negative value is then assigned, making it easy to select only events which actually pass the $t \bar t$ reconstruction.
...
After a trial and error process, the best results have actually been obtained with our \ac{ANN} over the \ac{BDT}, method therefore chosen for the rest of this analysis.%, considering as input a relatively simple set of variables, all described in Section~\ref{section:Variables}. By order of importance, we therefore decided to use as input the stransverse mass $M_{T2}^{ll}$, the angle $\Delta \Phi(E_{T}^{\text{miss}}, ll)$, the pf\ac{MET}, $m_{bl}^t$, one of the two spin correlated variables $\xi = \cos(\theta_i) \cos(\theta_j)$ and the number of b-jets observed. The extensive explanation behind the choice of all these parameters can be found in Appendix~\ref{appendix:Optimization}.

\subsection{Training process}

One training has been per combination of mediator mass, considering separately the scalar and pseudoscalar mediators in order to enhance the sensitivity of the analysis. Each \ac{ANN} has been trained in order to reduce as much as possible the dependence on the sample itself, by weighting the samples according to the specific cross-section of each mass point. 

For the training process itself, all the signals for a given mass point have been mixed together and trained against all the possible backgrounds, in the pre-selection region defined in Section~\ref{section:SR}. Two different trainings were then performed for each mass point:
\begin{itemize}
\item One targeting mostly the $t/\bar t$+DM signal, by considering only the events having exactly 1 jet or 2 jets and exactly 1 b-jet;
