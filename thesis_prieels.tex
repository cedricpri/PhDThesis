...
\begin{itemize}
\item The first \ac{DM} candidate are the so-called \textbf{\ac{MACHO}}. These objects are massive astronomical non-luminous bodies (such as black holes) made of baryonic matter that could be responsible of the gravitational lensing observed and that could explain the apparent missing mass in the Universe. However, as we saw in Section~\ref{section:DMProperties}, \ac{DM} is not expected to be made of such ordinary matter, basically because observational data of the \ac{CMB} and the deduced baryonic density of energy in the Universe is able to rule out this possibility.
\item Axions
\item Sterile neutrinos
\item Last but not least, WIMP. Hierarchy problem solved.
\end{itemize}
