...
The beam, injected from the \ac{SPS} into the \ac{LHC} into two counter-rotating beams, is separated in a series of 2808 bunches of protons, nominally separated by 25ns (giving raise to a collision rate of 40 MHz), so that the experiments have the time to record the collision and clean the detectors so that they are ready for the next bunch crossing, coming just a few nano seconds later. Each time one bunch crossing happens, around 25 collisions of protons happen at time: this phenomena, usually referred to as \ac{PU}, has to be taken into account as well and will be described in the next section.
...
N = \sigma \int_0^T \mathcal{L}(t) dt = \sigma L
\end{dcases}
\end{equation}
...
\begin{equation}
\label{eq:Luminosity}
\mathcal{L} = \frac{\gamma k_B N_p^2}{4 \pi \epsilon_n \beta^*}F
\end{equation}

In this last equation, the following properties of the accelerator have been introduced:
\begin{itemize}
...
\item $f$ is the frequency of revolution
\item $k_B$ is the number of proton bunches per beam
\item $N_p$ is the number of protons per bunch
\item $\epsilon_n$ is the normalized emittance, describing how much space the beam occupies in the phase space
\item $\beta^*$ is the betatron function at the interaction point
\item $F$ is the form factor accounting for the fact that the collisions do not happen exactly head-on
\end{itemize}
...
The total integrated luminosity taken by the \ac{LHC} during its different years of operation has been summarized in Figure~\ref{fig:Lumi}. \color{red} The final datasets available that will be analyzed in this work have an integrated luminosity of 35.9 fb$^{-1}$ (2016), 41.5 fb$^{-1}$ and 58.5 fb$^{-1}$ (2018), resulting in a total dataset of 138 fb$^{-1}$ for the Run II of operation. \color{black} This typically means that we expect to produce around 138 events of any process whose cross section of production would be equal to 1 fb.
...
\subsubsection{Pile Up} \label{subsection:PU}
