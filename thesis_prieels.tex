...
\item \textbf{Lepton trigger}. An uncertainty associated to the lepton trigger efficiency is taken into account. It has an impact of the order of $\sim 2$\%, estimated from a general Tag and Probe method, by varying the Z window considered and the selection applied to the tag lepton. 
\item \textbf{Lepton reconstruction and selection}. Scale factors are applied to the \ac{MC} processes in order to mimic the measured lepton reconstruction and selection efficiencies in data, and such scale factors come with estimated $p_T$ and $\eta$ uncertainties of the order of $\sim2$\% for electrons and muons.
...
\begin{itemize}
\item \textbf{Single top and diboson normalization}. 
\item \textbf{Non-prompt background}. A normalization and shape uncertainty is associated to the estimation of this background in a specific control region according to the tight-to-loose method pevious described. The impact of a change in the input jet $E_T$ threshold chosen has been estimated to be less than $\sim10$\%, while a flat 30\% systematic is considered for this background to cover for eventual discrepancies between data and \ac{MC} in the same-sign control region defined.
\end{itemize}
