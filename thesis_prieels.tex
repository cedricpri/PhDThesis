...
\item \textbf{\ac{SM} neutrinos} have been considered as good \ac{DM} candidates for a long time as well. However, two physical reasons can explain why we do not really believe in this hypothesis any more. First of all, their relative abundance does not match the expected one for \ac{DM} from the freeze-out mechanism explained in Section~\ref{section:DMProperties}. Indeed, their freeze-out abundance can be computed quite easily from Equation~\ref{equation:neutrinoRelic} \cite{WIMPBook}, where the sum of the masses of the three neutrino flavors has been calculated in 2017 to be lower than 0.17 eV at the 95\% \ac{CL} from cosmological studies \cite{NeutrinoMass}, giving us an abundance much lower than the measured one for \ac{DM} from the studies of the \ac{CMB}, for example. 
...
\item The actual \ac{DM} candidate that will be mostly considered throughout this work are the so-called \textbf{\acf{WIMP}}, which are expected to interact, even though very weakly, with ordinary baryonic matter and which have an expected mass in the range of 100 GeV to 1 TeV for reasonable electroweak production cross-section values, right where we expect \ac{DM} to be found from its relic density: this is the so-called "WIMP miracle". Several strategies can be used to detect such particles, as we will now see in the Section~\ref{section:DMSearches}. This kind of particle basically arises in various \ac{BSM} theories, such as the lightest supersymmetric particle in SUSY. According to this theory, each \ac{SM} particle should have a superpartner whose quantum numbers would be identical except for their spins, which would differ by one half. All of these superpartners would then be new and undiscovered particles, giving us a perfect \ac{DM} candidate, the neutralino $\chi$.
...
The indirect detection of \ac{DM} particles consists basically in searching in space for \ac{SM} products coming from the annihilation of two \ac{DM} particles or from its eventual direct decay. In this case, this interaction can be probed by both observatories placed either on the ground or in space directly. Many extensions to the \ac{SM}, such as SUSY or \ac{UED} do provide \ac{DM} candidates (the lightest supersymmetric particle and the lightest Kaluza-Klein state, respectively) expected to interact with each other, resulting in the immediate production of \ac{SM} (un)stable particles that we could detect.

Because of the freeze-out scenario explained in Section~\ref{section:DMProperties}, we can find an expression relating the actual abundance of \ac{DM} $\Omega_\chi$ with its through Equation~\ref{eq:Annihilation} \cite{IndirectSearches}, where the quantity $\langle \sigma_A v \rangle$ corresponds to the \ac{DM} annihilation cross section.

\begin{equation}
\label{eq:Annihilation}
\Omega_\chi h² \sim \frac{3 \cdot 10^{-27} cm^3 s^{-1}}{\langle \sigma_A v \rangle}
\end{equation}

This equation implies that, since we do not the current abundance of \ac{DM} in the Universe, the total annihilation cross section of \ac{DM} should be equal to $\sim 3 \cdot 10^{-26} cm^3 s^{-1}$, the typical value given by \ac{WIMP}s, as discussed previously. The experiments trying to indirectly detect \ac{DM} therefore mostly focus on \ac{WIMP} searches for this reason. Sterile neutrinos can also be searched indirectly, since they are expected to decay into active neutrinos, producing a photon at exactly half the sterile neutrino mass, which could be detected as well. Several strategies can be used in order to detect \ac{DM} indirectly:

\begin{itemize}
\item The golden channel for such searches are through the production of \textbf{$\gamma$-rays}, mainly because the energy scale of the \ac{WIMP}s implies that most of the annihilation and decay radiation will be emitted in this range of energies, and because $\gamma$-rays are usually not deflecting when traveling to the observer. The flux of such $\gamma$-rays received within a solid angle $\Delta \Omega$ depends on two factors, as described in Equation~\ref{eq:Flux}: the first one depending on the physics itself (such as the \ac{DM} properties), while the second one, also called J-factor, depends only on the distribution of \ac{DM} within the system considered, where $l$ is the line of sight considered \cite{JFactor}.

\begin{equation}
\label{eq:Flux}
\begin{dcases}
\frac{d \phi_\gamma}{dE_\gamma} = \phi^{PP}(E) \cdot J(\Delta \Omega) \\
J = \iint \rho_{DM}^2(l,\Omega) \text{ } dl \text{ } d\Omega
\end{dcases}
\end{equation}

This J-factor is then actually a measurement of the quality of a astronomical object for an indirect measurement, since the higher the flux received, the better the measurement will be in general (even though this is not the only factor which matters, since for example the galactic center has a higher J-factor than the best dwarf galaxy observable, but also has a lot of backgrounds affecting the measurement).

As we just saw, $\gamma$-rays are perfect candidates for the indirect detection of \ac{DM} but they do have one drawback as well: the Earth's atmosphere is usually opaque to this kind of radiation. This means that most of experiments searching for them have to be sent to space. QUOTE EXAMPLES
...
\subsection{Trigger}DirectSearches
...
\href{https://arxiv.org/pdf/1602.00039.pdf}{P.W. Graham et all.,
"Experimental Searches for the Axion and Axion-like Particles",
Annual Review if Nuclear and Particle Science 65, 2015
}
...
\bibitem{JFactor}
\href{https://arxiv.org/abs/1604.05599}{N.W. Evans et all.,
