...
This background is also estimated from \ac{MC} but a correction factor obtained from a general $R_{\text{in-out}}$ method is applied to this particular process, which is additionally being checked in a specific control region as well.

\subsection{$t \bar t + W$/$t \bar t$+Z} \label{subsection:ttV}

These backgrounds are coming from a usual $t \bar t$ production along with an \acf{ISR} or \ac{FSR} production of a W or Z boson, as shown in Figure~\ref{fig:ttV}. The contribution of both these backgrounds is also taken directly from \ac{MC}.
...
\subsubsection{$R_{\text{in/out}}$ correction} \label{subsection:rinout}

\color{red} FIXME: describe the method \color{black}
