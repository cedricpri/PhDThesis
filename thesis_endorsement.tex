...
This channel is then \textbf{expected to be competitive with the hadronic channel}, especially when considering high mediator masses, which feature a higher global signal/background discrimination. \vfill
\end{frame}

\begin{frame}{Previous relevant results I}
\justifying
\vspace{5pt}
A similar analysis has already been carried out by CMS using 2016 data, considering the $t \bar t$+DM signal only and the dilepton final state only (AN-2016-478). \vfill % \cite{PreviousDoubleTopAllLep13CMS}. \vfill
...
The observed (expected) limits \textbf{excluded a scalar mediator} with mass below 80 (70) GeV, while \textbf{no exclusion was achieved} when considering pseudoscalar mediators. \vfill
...
They obtained expected scalar (pseudoscalar) exlusion limits of 250 (300) GeV, even though  they used NLO cross-sections for the signals, around 30\% higher than ours. \vfill
\end{frame}
...
Given that my PhD thesis is now coming to an end, with no further extensions possible, I would like to ask the conveners to endorse the results presented next and also fully documented in the AN-2022-014. \vfill
\end{frame}
...
\end{columns} \vfill
\vspace{5pt}
A \textbf{blinding policy} was followed at first, allowing us to only look at $1$~fb$^{-1}$ of data per year near the signal regions. \vfill
\vspace{10pt}
...
\item Others, such dibosons and tribosons production, all taken from MC.
\end{itemize} \vfill
\end{frame}
...
\item Trigger $p_T$ carefully chosen to avoid any turn-on effect;
...
\item Additional small cuts on the impact parameters to reduce the non-prompt contamination in the MET tail ($|d_0| < 0.05$ cm, $|d_z| < 0.1$ cm, $S_{3D}^d < 4$).
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Leading (trailing) opposite sign lepton $p_T >$ 25 (20) GeV;
...
A pre-selection region is then defined by asking for:
\begin{itemize}
\item At least 1 medium deep CSV b-jet;
\item A 15 GeV Z-veto on the $ee$ and $\mu \mu$ channels;
\item MET $>$ 100 GeV and $M_{T2}^{ll} >$ 80 GeV to keep this region orthogonal to the $t \bar t$ control regions used by the semi-leptonic channel.
\end{itemize} \vfill
...
\begin{columns}
		\begin{column}{0.33\textwidth}
			...
			\vspace{-8pt}
			\begin{block}{\centering 2016}\end{block}\vspace{10pt}
     			\includegraphics[width=1.0\textwidth, height=90pt]{figs/2016/SmearSR-ttDM-scalar100/log_cratio_topCR_ll_METcorrected_pt.png}
    		\end{center}		
		\end{column} 
		\begin{column}{0.33\textwidth}
			\begin{center}
			\vspace{-8pt}
			\begin{block}{\centering 2017}\end{block}\vspace{10pt}
     			\includegraphics[width=1.0\textwidth, height=90pt]{figs/2017/SmearSR-ttDM-scalar100/log_cratio_topCR_ll_METcorrected_pt.png}
    		\end{center}		
		\end{column} 
		\begin{column}{0.33\textwidth}
			\begin{center}
			\vspace{-8pt}
			\begin{block}{\centering 2018}\end{block}\vspace{10pt}
     			\includegraphics[width=1.0\textwidth, height=90pt]{figs/2018/SmearSR-ttDM-scalar100/log_cratio_topCR_ll_METcorrected_pt.png}
    		\end{center}		
		\end{column}
\end{columns}
\begin{columns}
		\begin{column}{0.33\textwidth}
			\begin{center}
				%\begin{block}{\centering Puppi MET}\end{block}	
     			\includegraphics[width=1.0\textwidth, height=90pt]{figs/2016/SmearSR-ttDM-scalar100/log_cratio_topCR_ll_mt2ll.png}
    		\end{center}		
		\end{column}
		\begin{column}{0.33\textwidth}
			\begin{center}
				%\begin{block}{\centering njet}\end{block}	
     			\includegraphics[width=1.0\textwidth, height=90pt]{figs/2017/SmearSR-ttDM-scalar100/log_cratio_topCR_ll_mt2ll.png}
    		\end{center}		
		\end{column}
		\begin{column}{0.33\textwidth}
			\begin{center}
				%\begin{block}{\centering nbjet}\end{block}	
     			\includegraphics[width=1.0\textwidth, height=90pt]{figs/2018/SmearSR-ttDM-scalar100/log_cratio_topCR_ll_mt2ll.png}
    		\end{center}		
		\end{column}
\end{columns} \vfill
\end{frame}
...
\item A typical SM $t \bar t$ event will given two ellipsed close to each other, while ellipses coming from a typical signal event will be further apart and will not intersect.
\end{itemize} \vfill
...
\item A \alert{smearing process} was followed to evaluate the impact that imperfectly measured kinematic variables can have on this process (100 iterations per event, by updating several parameters such as the energy/direction of the jets and leptons within their uncertainties).
\end{itemize} \vfill
...
\begin{itemize}
\item One targeting the $t/\bar t$+DM signal, by considering events having excactly 1 jet, or exactly 2 jets and 1 b-jet;
...
\justifying
The hyperparameters of the BDT \textbf{have been fully optimized} one by one, trying each time to minimize the error in the test dataset and the discrimination obtained. \vfill
...
\alert{Recommanded correction factors} (L1 ECAL prefiring in 2016 and 2017, HEM issue in 2018) are then applied to the simulation. \vfill 
\end{frame}

\begin{frame}{Top control region}
\justifying
Same as the pre-selection region but with MET $> 50$ GeV and $60 < M_{T2}^{ll} < 80$ GeV. \vfill
...
\vspace{-5pt}
A flat scale factor and a fixed 20\% systematic uncertainty is then applied to the DY. This \vfill
\end{frame}
...
\begin{itemize}
\justifying
\item PDF and higher order corrections, underlying event and parton shower modeling, renormalization and factorization scales.
\end{itemize} \vfill
...
\begin{itemize}
\justifying
\item Luminosity, pileup modeling, lepton trigger, lepton efficiency and energy scale, jet energy scale, MET mismodelling, b-tagging efficiency, top $p_T$ reweighting, ECAL prefiring.
\end{itemize} \vfill
...
\begin{figure}[htbp]
\centering
\begin{block}{\centering 2016}\end{block}	\vspace{-8pt}
...
After combination of the different years, an expected (observed) exclusion of:
\begin{itemize}
\item Scalar mediators is achieved up to 155 (130) GeV;
\item Pseudoscalar mediators is obtained up to 150 (105) GeV. 
\end{itemize} \vfill
\end{frame}
...
This search improves by a factor of 2 the scalar exclusion limits obtained in 2016 and manages to exclude for the first time pseudoscalar mediators up to 150 GeV. \vfill
...
\justifying
CMS combination of all the different final states published in 2016: \vfill
...
\begin{itemize}
\justifying
\item $r2l$, defined as the ratio between the MET and the $p_T$ of the two leptons observed; \item And $r2l4j$, defined in a similar way but considering additionally the $p_T$ of the first 4 jets (if they exist) in the sum in the denominator.
\end{itemize} \vfill
